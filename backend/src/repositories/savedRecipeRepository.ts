...
export const savedRecipeRepository = {
  getSavedRecipesByUserId(userId: number): Promise<IRecipeDomainModel[]> {
    return db.query<IRecipeDomainModel[]>(
      `SELECT recipe.id as recipeId, recipe.recipe_name as recipeName, recipe.description, recipe.instruction, recipe.image_id as imageId
      FROM recipe
      JOIN saved_recipe ON recipe.id = saved_recipe.recipe_id
